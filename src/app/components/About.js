...
export default function About({ data }) {
  return (
    <div className='  w-full flex flex-col items-center justify-center gap-y-4' id='about'>
        <span className='border-b-2 px-12 ml-24'></span>
        <p className='text-4xl font-serif tracking-widest drop-shadow-lg'>{data.name}</p>
        ...
        {data.image.map((elem,index)=>(
          <div className={`lg:w-3/4 lg:h-[500px] w-auto h-auto flex flex-col items-center justify-center lg:my-16 my-14 ${index % 2 === 0 ? "lg:flex-row" : "lg:flex-row-reverse"}`}   key={index}>
              <div className='w-3/4 h-[500px] bg-[#7b5d79] mx-20 rounded-lg shadow-xl shadow-gray-500'>
                <Image src={elem.img}
                width={elem.width}
                height={elem.height}
                alt='single'
                className='h-full object-cover cursor-pointer translate-12 rounded-lg shadow-xl shadow-gray-500' />
              </div>
